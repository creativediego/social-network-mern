...
const listenForNewTuits = (
  socket: Socket,
  dispatchAction: ThunkDispatch<unknown, unknown, AnyAction>
) => {
  socket.on('NEW_TUIT', (tuit) => {
    dispatchAction(addTuit(tuit));
  });
};

const listenForUpdatedTuits = (
  socket: Socket,
  dispatchAction: ThunkDispatch<unknown, unknown, AnyAction>
) => {
  socket.on('UPDATED_TUIT', (tuit) => {
    dispatchAction(updateTuits(tuit));
  });
};
...
export const enableListeners = (
  ...
  socket.once('connect', () => {
    ...
    listenForNewTuits(socket, dispatchAction);
    listenForUpdatedTuits(socket, dispatchAction);
    listening = true;
  });
